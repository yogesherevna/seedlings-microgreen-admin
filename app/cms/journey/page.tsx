...
export default function Journey(){
 ...
 async function save(e:React.FormEvent){e.preventDefault();if(!form)return;setSaving(true);try{const {id:_id,...p0}=form;const p={...p0,blockKey:selected};if(form.id)await setRecord("websiteJourneyContent",form.id,p,true);else await createRecord("websiteJourneyContent",p);await load()}catch{setError("Unable to save Journey content.")}finally{setSaving(false)}}
